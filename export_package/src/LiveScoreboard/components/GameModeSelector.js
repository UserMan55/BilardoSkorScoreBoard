...
const GameModeSelector = React.memo(({ focusedIndex, onFreeStart, onStandardStart, onSurvivalStart }) => {
  return (
    <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center' }}>
      ...
      <button 
        tabIndex="1"
        className={`mode-card free-mode ${focusedIndex === 1 ? 'focused' : ''}`}
        onClick={onFreeStart}
      >
      ...
      <button 
        tabIndex="2"
        className={`mode-card standard-mode ${focusedIndex === 2 ? 'focused' : ''}`}
        onClick={onStandardStart}
      >
        <img src="/logo.png" alt="3CScore" style={{ width: '30px', height: 'auto', marginBottom: '5px', objectFit: 'contain', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.2))' }} />
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <button 
        tabIndex="3"
        className={`mode-card survival-mode ${focusedIndex === 3 ? 'focused' : ''}`}
